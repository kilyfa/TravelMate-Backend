...
const postUserHandler = async (request, h) => {
  ...
  if (!file) {
    throw new InputError("Gambar profil harus disertakan");
  }

  const blob = bucket.file(`profiles/${nameId}-${file.hapi.filename}`);
  ...
  const profileUrl = `https://storage.googleapis.com/${bucket.name}/${blob.name}`;
